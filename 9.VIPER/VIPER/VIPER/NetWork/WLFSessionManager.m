...
@interface WLFSessionManager()

@property (nonatomic, strong) AFHTTPSessionManager *sessionManager;

@end

@implementation WLFSessionManager
...
- (void)request:(RequestType)requestType urlStr: (NSString *)urlStr parameter: (NSDictionary *)param resultBlock: (void(^)(id responseObject, NSError *error))resultBlock {
    ...
    if (requestType == RequestTypeGet) {
        [self.sessionManager GET:urlStr parameters:param headers:nil progress:nil success:successBlock failure:failBlock];
        
    }else {
        [self.sessionManager POST:urlStr parameters:param headers:nil progress:nil success:successBlock failure:failBlock];
